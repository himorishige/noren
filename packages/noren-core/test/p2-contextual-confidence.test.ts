...
describe('P2 Contextual Confidence', () => {
  ...
  describe('calculateContextualConfidence', () => {
    ...
    it('should suppress more for closer markers', () => {
      ...
      const closeResult = calculateContextualConfidence(closeHit, closeText, 0.8, DEFAULT_CONTEXTUAL_CONFIG)
      const neutralResult = calculateContextualConfidence(neutralHit, neutralText, 0.8, DEFAULT_CONTEXTUAL_CONFIG)
      ...
      expect(closeResult.explanations.some(exp => exp.ruleId === 'example-marker-strong')).toBe(true)
    })

    it('should suppress confidence in template contexts', () => {
      const templateText = 'Hello {{name}}, your email is: user@example.com'
      const result = calculateContextualConfidence(sampleHit, templateText, 0.8, DEFAULT_CONTEXTUAL_CONFIG)
...
\`\`\``
      const result = calculateContextualConfidence(sampleHit, codeText, 0.8, DEFAULT_CONTEXTUAL_CONFIG)
...
John,user@example.com,555-1234`
      const result = calculateContextualConfidence(sampleHit, csvText, 0.8, DEFAULT_CONTEXTUAL_CONFIG)
    ...
    it('should handle Japanese context markers', () => {
      ...
      const result = calculateContextualConfidence(hit, japaneseText, 0.8, DEFAULT_CONTEXTUAL_CONFIG)
...
\`\`\``
      ...
      const result = calculateContextualConfidence(sampleHit, complexText, 0.8, DEFAULT_CONTEXTUAL_CONFIG)
  ...
  describe('Configuration variations', () => {
    it('should use conservative config correctly', () => {
      const text = 'Example: user@example.com for testing'
      const result = calculateContextualConfidence(sampleHit, text, 0.8, CONSERVATIVE_CONTEXTUAL_CONFIG)
  ...
  describe('Performance and edge cases', () => {
    ...
    it('should handle malformed text gracefully', () => {
      ...
      const result = calculateContextualConfidence(sampleHit, malformedText, 0.8, DEFAULT_CONTEXTUAL_CONFIG)
