...
describe('Backpressure Handling Integration', () => {
  ...
  it('should handle large chunk processing with controlled memory usage', async () => {
    ...
    const avgProcessingMemory = memoryPeaks.reduce((sum, peak) => sum + peak.processingMemory, 0) / memoryPeaks.length
    ...
    console.log(`  Throughput: ${Math.round(processedBytes / (processingTime / 1000) / (1024 * 1024))}MB/s`)
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      const redactionCount = (result.match(/\[REDACTED:/g) || []).length
      assert.ok(redactionCount > 100, `Large chunk ${i} should have many redactions (got ${redactionCount})`)
    }
  })
  ...
  it('should handle backpressure in concurrent processing scenarios', async () => {
    ...
    const createConcurrentStream = (streamId: number) => {
      ...
      const readable = new ReadableStream({
        start(controller) {
          ...
          }, 10 + streamId * 5) // Slightly different intervals per stream
        }
      })
    ...
    for (const { streamId, results, stats } of streamResults) {
      console.log(`  Stream ${streamId}: ${results.length} results, ${stats.processedCount} processed, ${stats.backpressureCount} backpressure events`)
      ...
      for (const result of results) {
        assert.ok(result.includes(`Stream${streamId}:`), `Result should be tagged with stream ${streamId}`)
    ...
    console.log(`  Average processing rate: ${Math.round(totalResults / (totalProcessingTime / 1000))} results/sec`)
    ...
    assert.equal(streamResults.length, numConcurrentStreams, 'All concurrent streams should complete')
    
    // Memory usage should be reasonable for concurrent processing
    assert.ok(memoryGrowth < 500 * 1024 * 1024, 'Concurrent processing memory growth should be reasonable (<500MB)')
    
    // Should complete in reasonable time despite backpressure
    assert.ok(totalProcessingTime < 30000, 'Concurrent processing should complete in reasonable time (<30s)')
  ...
  it('should recover gracefully from backpressure-induced errors', async () => {
    ...
    const faultyTransform = new TransformStream({
      async transform(chunk, controller) {
        ...
        try {
          ...
          try {
            controller.enqueue(`[ERROR_RECOVERED: Chunk processed with errors - ${text.length} bytes]`)
            recoveryCount++
          } catch (recoveryError) {
            controller.error(error)
          }
    ...
    if (results.length > 0) {
      ...
      if (recoveredResults.length > 0) {
        ...
        assert.equal(recoveredResults.length, recoveryCount, 'Recovery results should match recovery count')
      }
      
      if (successfulResults.length > 0) {
        // Verify successful results contain proper redactions
        for (const result of successfulResults.slice(0, 5)) { // Check first 5
