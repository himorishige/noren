 ...
 */
const SECURITY_LEVELS: Record<SecurityLevel, {
 ...
 */
async function ensureGuard(config?: SimpleConfig): Promise<Awaited<ReturnType<typeof createLazyGuard>>> {
...
export function createExpressMiddleware(config?: SimpleConfig) {
  return async (req: any, res: any, next: any) => {
    try {
      const body = req.body?.message || req.body?.content || req.body
      if (typeof body === 'string') {
        ...
        if (config?.sanitize !== false) {
          req.body.sanitized = await sanitize(body, config)
        }
      }
...
export async function checkRequest(
  ...
  try {
    ...
    const sanitized = config?.sanitize !== false 
      ? await sanitize(body, config)
      : undefined
...
export async function createGuard(type: 'financial' | 'healthcare' | 'general' | 'security') {
  const configs = {
    financial: { level: 'strict' as SecurityLevel, categories: ['core', 'financial', 'security'] as PatternCategory[] },
    healthcare: { level: 'strict' as SecurityLevel, categories: ['core', 'personal', 'security'] as PatternCategory[] },
    security: { level: 'strict' as SecurityLevel, categories: ['core', 'security'] as PatternCategory[] },
