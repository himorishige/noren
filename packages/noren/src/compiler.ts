 ...
 */
function _encodeString(str: string): Uint8Array {
  ...
  for (let patternIdx = 0; patternIdx < patterns.length; patternIdx++) {
    ...
    for (const literal of literals) {
      ...
      for (const char of literal) {
        ...
        const nextNode = current.children.get(charCode)
        if (!nextNode) throw new Error('Pattern compilation error')
        current = nextNode
      }
...
export function serializeCompiledPatterns(compiled: CompiledPatternSet): Uint8Array {
  ...
  // Header
  view.setUint32(offset, compiled.version, true)
  offset += 4
  view.setUint32(offset, compiled.patternCount, true)
  offset += 4
  view.setUint32(offset, compiled.stringTableSize, true)
  offset += 4
  view.setUint32(offset, compiled.trieNodes.length / 5, true)
  offset += 4 // trie node count
...
export function deserializeCompiledPatterns(data: Uint8Array): CompiledPatternSet {
  ...
  offset += 4
  const patternCount = view.getUint32(offset, true)
  offset += 4
  const stringTableSize = view.getUint32(offset, true)
  offset += 4
  const trieNodeCount = view.getUint32(offset, true)
  offset += 4
