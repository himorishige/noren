...
async function realisticBenchmark() {
  ...
  console.log(
    `│ ウォーム実行        │ ${warmAvg.toFixed(4)}ms    │ ${warmQPS.toFixed(0).padStart(6)} QPS   │`,
  )
  console.log(
    `│ 大量バースト        │ ${burstAvg.toFixed(4)}ms    │ ${burstQPS.toFixed(0).padStart(6)} QPS   │`,
  )
  console.log(
    `│ プリロード最適化    │ ${preloadAvg.toFixed(4)}ms    │ ${preloadQPS.toFixed(0).padStart(6)} QPS   │`,
  )
  console.log('╰─────────────────────┴──────────────┴──────────────╯\n')
