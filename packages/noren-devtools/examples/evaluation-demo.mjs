...
async function basicEvaluationDemo() {
  ...
  for (const bucket of confAnalysis.confidence_buckets) {
    if (bucket.tp > 0 || bucket.fp > 0) {
      console.log(
        `  ${bucket.range}: Precision ${(bucket.precision * 100).toFixed(2)}% (TP: ${bucket.tp}, FP: ${bucket.fp})`,
      )
    }
  }
  ...
  for (const entryId of Object.keys(detectionResults)) {
    ...
    if (entryResult.true_positives.length > 0) {
      ...
      for (const tp of entryResult.true_positives) {
        console.log(
          `    ✓ ${tp.detected.type}: "${tp.detected.value}" (overlap: ${(tp.overlap_ratio * 100).toFixed(1)}%)`,
        )
      }
    }
...
function syntheticDatasetDemo() {
  ...
  const patterns = [
    ...
    { type: 'ip', patterns: ['192.168.1.1', '10.0.0.1', '172.16.0.1', '2001:db8::1', '::1'] },
    { type: 'credit_card', patterns: ['4111111111111111', '5555555555554444', '378282246310005'] },
  ]
  ...
  for (const { type, patterns: typePatterns } of patterns) {
    for (const pattern of typePatterns) {
      const entry = TestDatasetBuilder.createSyntheticEntry(`synthetic-${type}-${entryId}`, [
        { type, pattern },
      ])
  ...
  for (let i = 0; i < 5; i++) {
    ...
    for (const { type, patterns: typePatterns } of patterns) {
      if (Math.random() > 0.6) {
        // 40% chance to include each type
...
async function edgeCaseTestingDemo() {
  ...
  const edgeCases = [
    ...
    },
    {
      ...
      annotations: [{ start: 0, end: 27, type: 'email', value: 'contact@company-support.com' }],
    },
    {
      id: 'edge-formatting',
